...
void ImageMask::ApplyMask(ImageType& source_image, ImageType& output_image, bool flag_normalize)
{
    ...
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < M; j++)
        {
            ...
            if (weighted_sum < -255)    // put a cap on negative values to prevent overflow
                weighted_sum = -255;
...
void ImageMask::ApplyPrewittX(ImageType& source_image, ImageType& output_image)
{
    ...
    //     {
    //         int val = 0;
    //         output_image.getPixelVal(i, j, val);
    //         std::cout << val << " ";
    //     }
    // }
}
...
void ImageMask::ApplySobelMag(ImageType& source_image, ImageType& output_image)
{
    ...
    ApplySobelX(source_image, SobelX);
    ApplySobelY(source_image, SobelY);

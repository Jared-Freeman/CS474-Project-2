...
void ImageMask::ApplyMedianFilter(ImageType& source_image, ImageType& output_image)
{
    //init source image vals
    int N, M, Q, median;
    source_image.getImageInfo(N, M, Q);
...
{
    std::vector<int> medianStorage; // hold px_vals to calc median
    int ox_s, ox_e, oy_s, oy_e; //offsets (start, end indices for x and y)
    int xm_start = 0, ym_start = 0;
    ...
    for (int k = ox_s, x = xm_start; k <= ox_e; k++, x++)
    {
        ...
        {
            ref_image.getPixelVal(k, l, px_val);
            medianStorage.push_back(px_val);
        }
    }
    // sort elements so that median is in the middle
